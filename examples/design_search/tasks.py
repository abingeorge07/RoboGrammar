...
# Added by BU: new task class for highest
# Create class for highest jumping task
class HighJumpTask(ABC):
  def __init__(self, time_step=1.0/240, discount_factor=0.99, interval=16,
               horizon=16, episode_len=128, noise_seed=0, force_std=0.0,
               torque_std=0.0):
    self.time_step = time_step
    ...
    self.objective_fn = rd.HeightObjective()
    self.objective_fn.base_dir_weight = np.array([0.0, 0.0, 0.0])
    self.objective_fn.base_up_weight = np.array([0.0, 2.0, 0.0])
    self.objective_fn.base_vel_weight = np.array([0.0, 2.0, 0.0])
    self.objective_fn.time_step = self.time_step

    self.objective_fn.height_weight = 8.0         # reward being high
...
# Added by BU: new task classes for various terrains
class FlatTerrainJumpTask(HighJumpTask):
  """
